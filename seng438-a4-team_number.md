...
We chose amazon page to test. First of all we tried to login with invalid email, then with invalid password, then with valid email and password in order to test all possible test data. After that we added an Item to the cart, then removed it. We then created a wish list, added an item to it, removed an item from it, then deleted the wish list. All tests passed as expected, however we removed some tests as we only needed 8 and some test cases we not performing as expected some times.
