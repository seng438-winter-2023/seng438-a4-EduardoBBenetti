...
1) **Changed conditional boundary → SURVIVED** --> expand() (333)          
This function is currently surviving changes in the boudary condition because when changing the operator from > to >=, in none of the test cases are able to spot this change. With that said, its necessary to have a JUnit test to kill this mutation in case the Range object has a lower and upper bounds place on the same values.  

2) **Negated double local variable number 1 → SURVIVED** --> intersects() (161)           
...
3) **Incremented (a++) double local variable number 1 → SURVIVED** --> Range() (95)          
...
4) **Decremented (a--) double local variable number 1 → SURVIVED** --> Range() (95)           
...
5)  **Incremented (a++) integer local variable number 3 → SURVIVED** --> hashCode() (464)        
...
6) **Decremented (a--) integer local variable number 3 → SURVIVED** --> hashCode() (464)             
