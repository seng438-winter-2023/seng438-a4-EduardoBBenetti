...
We chose amazon page to test. First of all we tried to login with invalid email, then with invalid password, then with valid email and password in order to test all possible test data. After that we searched for valid and invalid items. Then we added an Item to the cart, then removed it. We then created a wish list, added an item to it, removed an item from it, then deleted the wish list. All tests passed as expected, however we removed some tests as we only needed 8 and some test cases we not performing as expected some times (We are assuming it's a problem in the software or the laptop performing the process). A readme file was uploaded to navigate through the test cases.

# Explain the use of assertions and checkpoints
Assertions and Checkpoints are essential tools in Selenium. Assertions allow developers to verify expected outputs by comparing the actual results of a test case with what was expected. This helps to detect issues or bugs in the application code. Selenium's Assert class provides various methods such as assertEquals(), assertTrue(), and assertFalse() to implement assertions. On the other hand, checkpoints verify the current state of a web page or application during test execution. They are used to verify web page properties such as the presence of a specific web element, the value of a field, or the URL of a web page. WebDriver class methods, including getTitle(), getCurrentUrl(), and getText(), can be used to implement checkpoints. In summary, assertions and checkpoints help ensure reliable and accurate web application testing and assist in identifying and resolving application issues.

# how did you test each functionaity with different test data
For some functionalities (such as creating a wishlist or deleting it) it wasn't possible to test with different test data as there was only one action to be performed with no other altenatives such as SignOut. For other functionalities it was possible. For login functinality, we tested it using invalid email, invalid password, and valid email and password. For searching for items in the cart, we tried searching for valid items where it gives us results, and invalid items where it displays no results.
...
One of the difficulties we faced was setting up the environment for part 1 as we were not sure where to import the test files. One of the challenges we also faced was working with Selenium, for some of the test cases it waited for the user to hover the mouse to a specific place in order to continue with the process, we believe that this might be an issue with the software or the laptop that's performing the process as we repeated the test cases more than once and it still did the same action. Lessons learned was working better as a group, communicating better, and finishing things early.
